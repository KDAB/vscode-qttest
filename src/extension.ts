...
class KDABQtTest {
  ...
  public defaultDebuggerTypeForPlatform(): string {
    ...
    if (os.platform() === "linux") {
      ...
      return DEBUGGER_MS_LLDB;
    } else {
      return DEBUGGER_MS_MSVC;
    }
  }

  /// Asks the user to pick a launch from a picker
  public async pickDebuggerConfFromLaunch(): Promise<
  ...
  > {
    try {
      ...
      if (launches.length === 0) {
        this.log(
          "ERROR: pickDebuggerConfFromLaunch: No launch configurations found",
        );
        return undefined;
      }
      ...
      let result = await vscode.window.showQuickPick(launchNames, {
        placeHolder: "Select debugger",
      });
      ...
      this.log(
        "INFO: pickDebuggerConfFromLaunch: User selected: " +
          JSON.stringify(launches[index]),
      );
  ...
  public async maybeRebuild(executableFileName: string): Promise<boolean> {
    try {
      ...
      if (!projects || projects.length === 0) {
        this.log(
          "ERROR: maybeRebuild: Failed to get projects for executable: " +
            executableFileName,
        );
        return false;
      }
      ...
      for (let project of projects) {
        ...
        if (!targetName) {
          ...
          if (targetName) {
            ...
            this.log(
              "ERROR: maybeRebuild: Failed to get target name for executable: " +
                executableFileName +
                "; codemodel was:\n" +
  ...
  public maybeWarnOfMissingDebugger(type: string): boolean {
    ...
    if (!ext) {
      ...
      vscode.window
        .showErrorMessage(msg, { modal: true, detail: detail }, "Open settings")
        .then((value) => {
  ...
  public async debuggerConf(): Promise<vscode.DebugConfiguration> {
    ...
    if (option === "Existing Launch") {
      ...
      if (launch) {
        ...
        thisExtension.log(
          "ERROR: debuggerConf: Failed to get debugger conf from launch, defaulting.",
        );
        option = this.defaultDebuggerTypeForPlatform();
      }
    }

    let dbgConf: vscode.DebugConfiguration = {
      name: "",
      request: "launch",
      type: "",
      program: "",
    ...
    this.log(
      "INFO: debuggerConf: Using debugger conf: " +
        dbgConf +
        "; option=" +
        option,
    );
  ...
  public async discoverAllTestExecutables(controller: vscode.TestController) {
    ...
    if (buildDirs.length === 0) {
      this.log(
        "ERROR: discoverAllTestExecutables: Failed to figure out cmake build directory",
      );
      return;
    }
    ...
    for (let buildDir of buildDirs) {
      ...
      this.log(
        "INFO: discoverAllTestExecutables: Found " +
          this.qttests.qtTestExecutables.length +
          " executables in " +
          buildDir,
      );
      ...
      if (this.checkTestLinksToQtTestLib()) {
        ...
        this.log(
          "INFO: discoverAllTestExecutables: Removing non-Qt tests, if any",
        );
      ...
      if (this.qttests.qtTestExecutables.length === 0) {
        this.log(
          "ERROR: discoverAllTestExecutables: No QtTest executables were found",
        );
        return;
      }
      ...
      for (var executable of this.qttests.qtTestExecutables) {
        this.log(
          "INFO: discoverAllTestExecutables: Found: " + executable.filename,
        );
  ...
  async projectsForExecutable(
    ...
    for (let folder of workspaceFolders) {
      ...
      if (!proj) {
        this.log(
          "WARN: projectsForExecutable: No CMake project is open. Maybe run configure first.",
        );
        continue;
      }
      ...
      model.configurations.forEach((conf) => {
        ...
        if (targetName) {
          ...
          if (targetName) {
            ...
            this.log(
              "WARN: projectsForExecutable: not found even with workaround",
            );
          }
        }
      });
    ...
    if (projects.length === 0) {
      this.log(
        "WARN: projectsForExecutable: No projects found for executable: " +
          executableFileName,
      );
    }
  ...
  async cppFileForExecutable(
    ...
    for (let folder of folders) {
      ...
      if (!proj) {
        this.log(
          "WARN: cppFileForExecutable: No CMake project is open. Maybe run configure first.",
        );
        continue;
      }
      ...
      model.configurations.forEach((conf) => {
        cmake
          .cppFilesForExecutable(executableFileName, conf, workaround)
          .forEach((cppFile) => {
    ...
    if (candidates.length === 0) {
      ...
      if (workaround) {
        ...
        return this.cppFileForExecutable(
          executableFileName,
          /*workaround=*/ true,
        );
      }
  ...
  public async parseTestsInExecutable(
    ...
    try {
      ...
      this.log(
        "ERROR: parseTestsInExecutable: Failed to parse slots in: " +
          testExecutable.filename,
      );
      return;
    }
    ...
    if (!testExecutable.slots) {
      this.log(
        "WARN: parseTestsInExecutable: Executable does not have any slots: " +
          testExecutable.filename,
      );
      return;
    }
    ...
    this.log(
      "INFO: parseTestsInExecutable: Found " +
        testExecutable.slots.length +
        " slots in " +
        testExecutable.filename,
    );
  ...
  public async cmakeBuildDirs(): Promise<string[]> {
    ...
    if (!cmakeExt) {
      this.log(
        "ERROR: cmakeBuildDir: ms-vscode.cmake-tools extension is not installed",
      );
      return [];
    }
  ...
  public async debugTest(
    ...
    return await new Promise(async (resolve, reject) => {
      ...
      if (!result) {
        this.log(
          "ERROR: while running debugger. conf=" + JSON.stringify(debuggerConf),
        );
        resolve();
        return;
      }
...
async function runHandler(
  ...
  while (queue.length > 0 && !token.isCancellationRequested) {
    ...
    let functionToRun = singleTestSlot
      ? singleTestSlot.runTest
      : qtTestExecutable?.runTest;
    if (!functionToRun) {
      continue;
    }
    ...
    try {
      ...
      if (shouldDebug) {
        ...
        let builtFine = await thisExtension.maybeRebuild(
      ...
      thisExtension.log(
        "Failed to run " + ourRunnable.command() + ": " + e.message,
      );
...
function updateStatusForSubTests(
  parentTest: QtTest,
  run: vscode.TestRun,
  success: boolean,
) {
  if (!parentTest.slots) {
    thisExtension.log(
      "INFO: updateStatusForSubTests: parentTest.slots is empty. Skipping",
    );
    return;
  }
    ...
    if (slot.lastTestFailure) {
      run.failed(
        ...
        new vscode.TestMessage(
          "Test failed on line " + slot.lastTestFailure.lineNumber,
        ),
      );
    } else {
    ...
    vscode.commands.registerCommand("KDAB.qttest.debugTest", async () => {
      ...
      if (executables.length > 1) {
        vscode.window.showWarningMessage(
          "More than one executable contains the slot, please run explicitly from the text explorer.",
        );
