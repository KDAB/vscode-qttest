...
suite("Debug Environment Variables", function () {
  ...
  suiteSetup(async function () {
    ...
    console.log("[debug-env-test] Setting configure preset to 'dev'...");
    await vscode.commands.executeCommand("cmake.setConfigurePreset", "dev");

    console.log("[debug-env-test] Running cmake.configure...");
    await vscode.commands.executeCommand("cmake.configure");

    console.log("[debug-env-test] Running cmake.build...");
    const buildResult = await vscode.commands.executeCommand("cmake.build");
    console.log(`[debug-env-test] cmake.build returned: ${buildResult}`);

    // Trigger only our extension's test discovery
    console.log("[debug-env-test] Refreshing tests...");
    ...
    console.log(
      `[debug-env-test] Setup complete. Found ${controller.items.size} test items.`,
    );
  });

  test("debugTest propagates ctest ENVIRONMENT when debugging slotB", async function () {
    this.timeout(60000);
    ...
    const qtTest = qtTestSlot.parentQTest;
    assert.ok(
      qtTest.environment.length > 0,
      "test1 should have ctest environment variables",
    );
    assert.ok(
      qtTest.environment.includes("MY_ENV=VALUE"),
      "test1 environment should include MY_ENV=VALUE",
    );

    // Verify the debug configuration includes the environment
    const debuggerConf = await thisExtension.debuggerConf();

    const command = qtTestSlot.command();

    // Call debugTest, which should propagate the environment
